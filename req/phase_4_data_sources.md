...
- 认证：OAuth 2.0（注册后获得 `client_id` + `client_secret`，换取 `access_token`）
- Base URL：`https://api.ibkr.com/v1/api/`
- 关键能力：
  - 历史行情：`/iserver/marketdata/history?conid=...&period=1Y&bar=1d`
  - 期权链：`/iserver/secdef/strikes` + `/iserver/secdef/info`
  - 基本面：`/iserver/fundamentals/...`（部分需订阅）
- 延迟数据：默认 15 分钟延迟（免费）；实时数据需市场数据订阅
- Token 刷新：access_token 有效期短，需通过 refresh_token 定期刷新

> **注意**：IBKR REST API 需要去 `developer.ibkr.com` 注册应用，审批通常 1-3 天。
