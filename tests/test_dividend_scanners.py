...
class TestScanDividendPoolWeekly:
    ...
    def test_scan_dividend_pool_weekly_computes_golden_price_tz_aware(self):
        """Weekly scan computes golden_price even when price DataFrame has tz-aware index (yfinance >= 0.2.28)."""
        dates = pd.date_range("2021-01-01", periods=60, freq="M", tz="UTC")
        prices = pd.Series([100.0 + i * 0.1 for i in range(60)], index=dates)
        price_df = pd.DataFrame({"Close": prices})
        ...
        assert len(results) == 1
        td = results[0]
        assert td.golden_price is not None, "golden_price should be computed even with tz-aware price index"
        assert td.golden_price > 0
